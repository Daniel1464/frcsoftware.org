...
A common stimulus for robot behavior is the buttons on a controller. For instance, shooting balls 
while the x button on the operator controller is held. From section 1A, you might recall that fetching 
...
Where the `getAButton()` method returns `true` if the button is being held, and `false` otherwise. So, it's
